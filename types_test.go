...
func (suite *TypesCustomTimestampTestSuite) TestUnmarshalCSVFilled() {
	...
	err := suite.testable.UnmarshalCSV(str)
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), "2020-09-10 15:15:15", suite.testable.Value().Format(CustomTimestampFormatDefault))
}
...
type TypesCustomDateTestSuite struct {
	suite.Suite
	testable *CustomDate
}

func (suite *TypesCustomDateTestSuite) SetupTest() {
	suite.testable = &CustomDate{}
}

func (suite *TypesCustomDateTestSuite) TestUnmarshalCSVFilled() {
	str := "2020-09-10"
	err := suite.testable.UnmarshalCSV(str)
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), "2020-09-10", suite.testable.Value().Format(CustomDateFormatDefault))
}

func (suite *TypesCustomDateTestSuite) TestUnmarshalCSVFilledWithSlash() {
	str := "09/10/2020"
	err := suite.testable.UnmarshalCSV(str)
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), "2020-09-10", suite.testable.Value().Format(CustomDateFormatDefault))
...
func (suite *TypesCustomDateTestSuite) TestUnmarshalCSVEmpty() {
	str := ""
	err := suite.testable.UnmarshalCSV(str)
	assert.NoError(suite.T(), err)
	assert.True(suite.T(), suite.testable.Value().IsZero())
...
func (suite *TypesCustomDateTestSuite) TestUnmarshalCSVError() {
	str := "foo"
	err := suite.testable.UnmarshalCSV(str)
	assert.Error(suite.T(), err)
...
func (suite *TypesCustomDateTestSuite) TestMarshalJSONSuccess() {
	suite.testable.Date = time.Date(2020, time.Month(1), 20, 0, 0, 0, 0, time.UTC)
	result, err := suite.testable.MarshalJSON()
	assert.Equal(suite.T(), []byte(`"2020-01-20"`), result)
	assert.Nil(suite.T(), err)
}

func (suite *TypesCustomDateTestSuite) TestMarshalJSONEmpty() {
	result, err := suite.testable.MarshalJSON()
	assert.Equal(suite.T(), []byte(`""`), result)
	assert.Nil(suite.T(), err)
}
...
func TestTypesCustomDateTestSuite(t *testing.T) {
	suite.Run(t, new(TypesCustomDateTestSuite))
}
